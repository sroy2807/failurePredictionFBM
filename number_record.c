...
#define Ns 1000
#define filename "number_bursts_record_beta5.0_L1e3.dat"
...
int main(){
	...
	double Tmax_Tot=0,Tfail_Tot=0,DeltaTau_Tot=0,P[Ns],Energy1,Energy,Energy_prev,flag1_tot,flag2_tot;
    	double stress_increment,absolute_stress,absolute_stress_tot,Nsurvivors_tot,Number_Tot,record_tot[Ns]; 
    	int breaking_fiber,sample,flag,record[Ns],Q[Ns];
	...
	flag1_tot=0.0;
	flag2_tot=0.0;
	for(Number=1;Number<Ns;Number++){record_tot[Number]=0;Q[Number]=0;}
	for(sample=0;sample<sample_max;sample++){
		for(Number=1;Number<Ns;Number++){record[Number]=0;}
		for(i=0;i<Ns;i++){
		        nnl[i]=i-1;
		        nnr[i]=i+1;
		    	}
		...
		for(Number=1;Number<Ns;Number++){record_tot[Number]+=record[Number];}
		}
	for(Number=1;Number<Ns;Number++){record_tot[Number]=((1.0*record_tot[Number])/(1.0*Q[Number]));}
	fp=fopen(filename,"a");
	for(Number=1;Number<Ns;Number++){fprintf(fp,"%d\t%f\n",Number,record_tot[Number]);}
	fclose(fp);
	return 0;
	}
