...
#define Ns 1000
#define filename2 "waiting_time_beta1.7_L1e3.dat"
...
int main(){
	...
	for(flag=0;flag<Ns;flag++){waiting1[flag]=((1.0*waiting[flag])/(1.0*Q[flag]));}
	fp2=fopen(filename2,"a");
	for(flag=0;flag<Ns;flag++){if(waiting[flag]>0){fprintf(fp2,"%d\t%d\t%f\t%f\n",flag,Q[flag],waiting[flag],waiting1[flag]);}}
	fclose(fp2);
	return 0;
	}
